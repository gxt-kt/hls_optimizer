...
void TestStream() {
	...
	hls::stream<typename WideType<t_DataType, (1 << t_LogParEntries)>::t_TypeInt> l_str;
	readVec2Stream<t_DataType, 1 << t_LogParEntries>(in, 16, l_str);
	writeStream2Vec<t_DataType, 1 << t_LogParEntries>(l_str,16,out);
	for(int i=0;i<20;i++) {
...
void Test() {
	...
	std::cout << "1 << t_LogParEntries=" << (1 << t_LogParEntries) << std::endl;
	using t_IndexType = unsigned int;

	// 创建输入向量并写入 HLS 流
	hls::stream<typename WideType<t_DataType, (1 << t_LogParEntries)>::t_TypeInt> l_str;
	readVec2Stream<t_DataType, 1 << t_LogParEntries>(p_x, p_n, l_str);
...
template <typename BLAS_dataType,int BLAS_matrixSizeA=100,int BLAS_matrixSizeB=100,int BLAS_matrixSizeC=100>
void fun(uint32_t p_m,
            uint32_t p_n,
            uint32_t p_k,
...
//	using BLAS_dataType = float;
    ...
    hls::stream<typename WideType<BLAS_dataType, BLAS_parEntries>::t_TypeInt> l_strC;
    hls::stream<typename WideType<BLAS_dataType, BLAS_parEntries>::t_TypeInt> l_strSum;
    gemmMatAMover<BLAS_dataType, 1>(p_A, p_m, p_n, p_k, l_strA);
...
//    gemm<BLAS_dataType, p_k, BLAS_parEntries>(p_m, p_n, p_k,l_strA,l_strB,l_strC);
//    gemm<BLAS_dataType, 1, BLAS_parEntries, 3>(p_m, p_n, p_k, 1, l_strA, l_strB, 1,
//                                                                   l_strC, l_strSum);

//    gemm<BLAS_dataType, 2, BLAS_parEntries, 100>(p_m, p_n,p_k, 1, l_strA, l_strB, 1,
//                                                                   l_strC, l_strSum);
